###fix for tommorow - fix probabilities every time a move is done
...
class MinesweeperAI():
    ...
    def __init__(self, height=8, width=8):
        ...
        self.mines = set()
        self.safes = set()
        self.probability = Probability(self.width,self.height,MINES)
    ...
    def add_knowledge(self, cell, count):
        ...
        self.probability.HandleWithSentence(sentence=new_sentence,known_safes=self.safes,known_mines=self.mines,explored_cells=self.moves_made)
        ...
        for cell in self.mines:
            self.probability.CellMine(cell)
        for cell in self.safes:
            self.probability.CellSafe(cell)
    ...
    def make_random_move(self):
        ...
        """
        min_value = 999
        for keys, values in self.probability.grid.items():
            if keys not in self.moves_made:
                if values < min_value:
                    min_value = values
                    min_keys = keys
        print(min_keys,min_value)
        return min_keys
